...
DOWNLOAD_FIRMWARE() {
    ...
    local LOCAL_FIRM_DIR="$(pwd)/Firmware/$MODEL"

    # Prefer firmware already stored locally under Firmware/<MODEL>/
    if [ -d "$LOCAL_FIRM_DIR" ] \
        && ls "$LOCAL_FIRM_DIR"/AP_*.tar.md5 >/dev/null 2>&1 \
        && ls "$LOCAL_FIRM_DIR"/CSC_*.tar.md5 >/dev/null 2>&1; then
        echo -e "======================================"
        echo -e "  Using local firmware"
        echo -e "======================================"
        echo -e "Source: $LOCAL_FIRM_DIR"
        rm -rf "$DOWN_DIR"
        mkdir -p "$DOWN_DIR"
        cp -a "$LOCAL_FIRM_DIR"/. "$DOWN_DIR"/
        return 0
    fi
...
APPLY_CUSTOM_FLOATING_FEATURE() {
    ...
    UPDATE_FLOATING_FEATURE "$FLOATING_FEATURE_FILE_DIRECTORY" "SEC_FLOATING_FEATURE_SAIV_SUPPORT_AI_REVITAL" "TRUE"
    UPDATE_FLOATING_FEATURE "$FLOATING_FEATURE_FILE_DIRECTORY" "SEC_FLOATING_FEATURE_SAIV_CONFIG_AI_REVITAL_VERSION" "1.9,1"
    UPDATE_FLOATING_FEATURE "$FLOATING_FEATURE_FILE_DIRECTORY" "SEC_FLOATING_FEATURE_SAIV_CONFIG_MIDAS" "UPSCALE,LOWLIGHT,HDR,DEBLUR,MOIRE,DENOISE_DEBLUR,UPSCALE_DEBLUR,PHOTOPOLISH,SHADOW,REFLECTION,UWDC,FLARE"
    ...
    local LAUNCHER_ANIM="LowEnd"
    if [ -n "${STOCK_ROM_FLOATING_FEATURE:-}" ] && [ -f "$STOCK_ROM_FLOATING_FEATURE" ]; then
        LAUNCHER_ANIM="$(GET_FF_VALUE "SEC_FLOATING_FEATURE_LAUNCHER_CONFIG_ANIMATION_TYPE" "$STOCK_ROM_FLOATING_FEATURE")"
        [ -z "$LAUNCHER_ANIM" ] && LAUNCHER_ANIM="LowEnd"
    fi
    UPDATE_FLOATING_FEATURE "$FLOATING_FEATURE_FILE_DIRECTORY" "SEC_FLOATING_FEATURE_LAUNCHER_CONFIG_ANIMATION_TYPE" "$LAUNCHER_ANIM"

    #========== GALLERY / GALAXY AI (Photo Assist menu) ==========#
    UPDATE_FLOATING_FEATURE "$FLOATING_FEATURE_FILE_DIRECTORY" "SEC_FLOATING_FEATURE_COMMON_SUPPORT_AI_AGENT" "TRUE"
    UPDATE_FLOATING_FEATURE "$FLOATING_FEATURE_FILE_DIRECTORY" "SEC_FLOATING_FEATURE_GALLERY_CONFIG_AI_EXPANSION" "AI_Timelapse"
    ...
    #========== GENAI (Photo Assist / Magic Eraser / Generative edit) ==========#
    UPDATE_FLOATING_FEATURE "$FLOATING_FEATURE_FILE_DIRECTORY" "SEC_FLOATING_FEATURE_GENAI_SUPPORT_C2PA" "TRUE"
...
# Re-apply stock display/performance tuning after custom floating features.
# Fixes A346E (120Hz) ported to A325F (90Hz) feeling sluggish/janky.
APPLY_STOCK_DISPLAY_PERF_TUNING() {
    if [ "$#" -ne 1 ]; then
        echo -e "Usage: ${FUNCNAME[0]} <FLOATING_FEATURE_FILE_DIRECTORY>"
        return 1
    fi
    ...
    if [ -z "${STOCK_ROM_FLOATING_FEATURE:-}" ] || [ ! -f "$STOCK_ROM_FLOATING_FEATURE" ]; then
        return 0
    fi

    local FLOATING_FEATURE_FILE_DIRECTORY="$1"

    echo "- Applying stock display/performance tuning for $STOCK_DEVICE."

    local HFR_RATES HFR_DEFAULT HFR_MODE HFR_NS LAUNCHER_ANIM
    HFR_RATES="$(GET_FF_VALUE "SEC_FLOATING_FEATURE_LCD_CONFIG_HFR_SUPPORTED_REFRESH_RATE" "$STOCK_ROM_FLOATING_FEATURE")"
    HFR_DEFAULT="$(GET_FF_VALUE "SEC_FLOATING_FEATURE_LCD_CONFIG_HFR_DEFAULT_REFRESH_RATE" "$STOCK_ROM_FLOATING_FEATURE")"
    HFR_MODE="$(GET_FF_VALUE "SEC_FLOATING_FEATURE_LCD_CONFIG_HFR_MODE" "$STOCK_ROM_FLOATING_FEATURE")"
    HFR_NS="$(GET_FF_VALUE "SEC_FLOATING_FEATURE_LCD_CONFIG_HFR_SUPPORTED_REFRESH_RATE_NS" "$STOCK_ROM_FLOATING_FEATURE")"
    LAUNCHER_ANIM="$(GET_FF_VALUE "SEC_FLOATING_FEATURE_LAUNCHER_CONFIG_ANIMATION_TYPE" "$STOCK_ROM_FLOATING_FEATURE")"

    [ -n "$HFR_RATES" ] && \
        UPDATE_FLOATING_FEATURE "$FLOATING_FEATURE_FILE_DIRECTORY" \
        "SEC_FLOATING_FEATURE_LCD_CONFIG_HFR_SUPPORTED_REFRESH_RATE" "$HFR_RATES"
    [ -n "$HFR_DEFAULT" ] && \
        UPDATE_FLOATING_FEATURE "$FLOATING_FEATURE_FILE_DIRECTORY" \
        "SEC_FLOATING_FEATURE_LCD_CONFIG_HFR_DEFAULT_REFRESH_RATE" "$HFR_DEFAULT"
    [ -n "$HFR_MODE" ] && \
        UPDATE_FLOATING_FEATURE "$FLOATING_FEATURE_FILE_DIRECTORY" \
        "SEC_FLOATING_FEATURE_LCD_CONFIG_HFR_MODE" "$HFR_MODE"
    [ -n "$HFR_NS" ] && \
        UPDATE_FLOATING_FEATURE "$FLOATING_FEATURE_FILE_DIRECTORY" \
        "SEC_FLOATING_FEATURE_LCD_CONFIG_HFR_SUPPORTED_REFRESH_RATE_NS" "$HFR_NS"
    [ -n "$LAUNCHER_ANIM" ] && \
        UPDATE_FLOATING_FEATURE "$FLOATING_FEATURE_FILE_DIRECTORY" \
        "SEC_FLOATING_FEATURE_LAUNCHER_CONFIG_ANIMATION_TYPE" "$LAUNCHER_ANIM"

    echo "- Stock refresh: ${HFR_DEFAULT:-unchanged}Hz (${HFR_RATES:-unchanged}), launcher: ${LAUNCHER_ANIM:-unchanged}"
}
...
APPLY_STOCK_ROM_FLOATING_FEATURE() {
    ...
    "$(GET_FF_VALUE "SEC_FLOATING_FEATURE_LCD_CONFIG_HFR_SUPPORTED_REFRESH_RATE_NS" "$STOCK_ROM_FLOATING_FEATURE")"
...
FIX_PHOTO_EDITOR_AI() {
    ...
    local EXTRACTED_FIRM_DIR="$1"
    local SYS="${EXTRACTED_FIRM_DIR}/system/system"
    local PE_APP="${SYS}/priv-app/PhotoEditor_AIFull"

    if [ ! -d "$PE_APP" ]; then
        return 0
    fi

    echo "- Fixing PhotoEditor AI models and runtime (crash fix)."

    # Short names in *.info must resolve to the real long filenames in the mod.
    # Use hard copies (not symlinks) so erofs/ext4 packing always includes them.
    _pe_link_model() {
        local dir="$1" short="$2" real="$3"
        if [ -f "$dir/$real" ]; then
            cp -f "$dir/$real" "$dir/$short"
        fi
    }

    if [ -d "${SYS}/etc/objectremoval/db" ]; then
        local ODB="${SYS}/etc/objectremoval/db"
        _pe_link_model "$ODB" "objecteraser.tflite" "12-12_PhotoEditor_ObjectEraser_v6.2.tflite"
        _pe_link_model "$ODB" "instanceseg.tflite" "12-28_PhotoEditor_ObjectEraser_v6.1_InstanceSeg.tflite"
        _pe_link_model "$ODB" "interactiveseg.tflite" "12-29_PhotoEditor_ObjectEraser_v4.2_InteractiveSeg.tflite"
        [ -f "$ODB/objecteraser.info" ] && \
            sed -i 's|objecteraser\.tflite|12-12_PhotoEditor_ObjectEraser_v6.2.tflite|g' "$ODB/objecteraser.info"
        [ -f "$ODB/instanceseg.info" ] && \
            sed -i 's|instanceseg\.tflite|12-28_PhotoEditor_ObjectEraser_v6.1_InstanceSeg.tflite|g' "$ODB/instanceseg.info"
        [ -f "$ODB/interactiveseg.info" ] && \
            sed -i 's|interactiveseg\.tflite|12-29_PhotoEditor_ObjectEraser_v4.2_InteractiveSeg.tflite|g' "$ODB/interactiveseg.info"
    fi

    if [ -d "${SYS}/etc/ailasso/db" ]; then
        local ADB="${SYS}/etc/ailasso/db"
        _pe_link_model "$ADB" "ailasso.tflite" "12-30_PhotoEditor_AILasso_v5.1.tflite"
        [ -f "$ADB/ailasso.info" ] && \
            sed -i 's|ailasso\.tflite|12-30_PhotoEditor_AILasso_v5.1.tflite|g' "$ADB/ailasso.info"
    fi

    if [ -d "${SYS}/etc/ailassomatting/db" ]; then
        local MDB="${SYS}/etc/ailassomatting/db"
        _pe_link_model "$MDB" "ailassomatting.tflite" "12-31_PhotoEditor_AILassoMatting_v7.0.tflite"
        [ -f "$MDB/ailassomatting.info" ] && \
            sed -i 's|ailassomatting\.tflite|12-31_PhotoEditor_AILassoMatting_v7.0.tflite|g' "$MDB/ailassomatting.info"
    fi

    if [ -d "${SYS}/etc/inpainting/db" ]; then
        local IDB="${SYS}/etc/inpainting/db"
        _pe_link_model "$IDB" "inpainting.tflite" "12-11_PhotoEditor_SpotFixer_v3.0.tflite"
        [ -f "$IDB/inpainting.info" ] && \
    ...
    if [ -f "${SYS}/etc/shadowremoval/db/shadow_removal.info" ]; then
        sed -i 's|shadow_removal\.info"|shadow_removal.tflite"|g' \
            "${SYS}/etc/shadowremoval/db/shadow_removal.info"
    fi
    if [ -f "${SYS}/etc/shadowremoval/db/shadow_maskextractor.info" ]; then
        sed -i 's|shadow_maskextractor\.info"|shadow_maskextractor.tflite"|g' \
            "${SYS}/etc/shadowremoval/db/shadow_maskextractor.info"
    fi

    # reflection_evaluator.info has a typo in model_path (evalutator)
    if [ -d "${SYS}/etc/reflectionremoval/db" ]; then
        local RDB="${SYS}/etc/reflectionremoval/db"
        for info in "$RDB"/*.info; do
            [ -f "$info" ] || continue
            sed -i 's|reflection_evalutator\.tflite|reflection_evaluator.tflite|g' "$info"
        done
    fi

    # style_transfer short names -> real PhotoEditor_* files
    if [ -d "${SYS}/etc/style_transfer/db" ]; then
        local SDB="${SYS}/etc/style_transfer/db"
        _pe_link_model "$SDB" "segmentation.tflite" "12-27_PhotoEditor_StyleTransfer_v3.0_Segmentation.tflite"
        _pe_link_model "$SDB" "color_pencil.tflite" "12-1_PhotoEditor_StyleTransfer_v3.0_ColorPencil.tflite"
        _pe_link_model "$SDB" "comic.tflite" "12-2_PhotoEditor_StyleTransfer_v3.0_Comic.tflite"
        _pe_link_model "$SDB" "watercolor.tflite" "12-3_PhotoEditor_StyleTransfer_v3.0_WaterColor.tflite"
        _pe_link_model "$SDB" "blue_ink.tflite" "12-4_PhotoEditor_StyleTransfer_v3.0_BlueInk.tflite"
        _pe_link_model "$SDB" "pastel.tflite" "12-5_PhotoEditor_StyleTransfer_v3.0_Pastel.tflite"
        _pe_link_model "$SDB" "marker.tflite" "12-6_PhotoEditor_StyleTransfer_v3.0_Marker.tflite"
        _pe_link_model "$SDB" "line_art.tflite" "12-7_PhotoEditor_StyleTransfer_v3.0_LineArt.tflite"
...
PY
    ...
    # Allow other processes to load ArcSoft object-capture libs
    local ARCSOFT_LIBS="${SYS}/etc/public.libraries-arcsoft.txt"
    if [ -f "${SYS}/lib64/libobjectcapture.arcsoft.so" ]; then
        touch "$ARCSOFT_LIBS"
        grep -qxF "libobjectcapture.arcsoft.so" "$ARCSOFT_LIBS" || \
            echo "libobjectcapture.arcsoft.so" >> "$ARCSOFT_LIBS"
        grep -qxF "libobjectcapture_jni.arcsoft.so" "$ARCSOFT_LIBS" || \
            echo "libobjectcapture_jni.arcsoft.so" >> "$ARCSOFT_LIBS"
    fi

    # Ensure priv-app permissions file is present
    if [ ! -f "${SYS}/etc/permissions/privapp-permissions-com.sec.android.mimage.photoretouching.xml" ]; then
        echo "- WARNING: PhotoEditor privapp permissions XML missing."
    fi

    VERIFY_PHOTO_EDITOR_AI "$EXTRACTED_FIRM_DIR"

    echo "- PhotoEditor AI crash fixes applied."
}


# Build-time check: Magic Eraser / Photo Assist files must exist before image build.
VERIFY_PHOTO_EDITOR_AI() {
    local EXTRACTED_FIRM_DIR="$1"
    local SYS="${EXTRACTED_FIRM_DIR}/system/system"
    local PE_APK="${SYS}/priv-app/PhotoEditor_AIFull/PhotoEditor_AIFull.apk"
    local MISSING=0

    echo "- Verifying Photo Assist / Magic Eraser payload."

    if [ ! -f "$PE_APK" ]; then
        echo "  FAIL: PhotoEditor_AIFull.apk missing"
        MISSING=1
    else
        echo "  OK: PhotoEditor_AIFull.apk"
    fi
    ...
    do
        if [ -f "$model" ]; then
            ...
            echo "  FAIL: missing $(basename "$model")"
            MISSING=1
        fi
    done
...
PY
    ...
    if [ -f "${SYS}/etc/permissions/privapp-permissions-com.sec.android.mimage.photoretouching.xml" ]; then
        ...
        echo "  FAIL: PhotoEditor privapp permissions XML"
        MISSING=1
    fi

    if [ "$MISSING" -ne 0 ]; then
        echo "- ERROR: Photo Assist verification failed. Magic Eraser may crash or not appear."
        echo "  Ensure internet during build so PhotoEditor AI Full mod downloads."
        return 1
    fi

    echo "- Photo Assist verification passed (Magic Eraser ready)."
    return 0
}
...
ADD_SAMSUNG_FLAGSHIP_APPS() {
    ...
    mkdir -p "$(pwd)/QuantumROM/Mods/Apps"

    # ================= SMART MANAGER =================
    echo "- Adding China smart manager."

    local SM_ZIP="$(pwd)/QuantumROM/Mods/Apps/Samsung_SmartManagerCN_Android_${ANDROID_VERSION}.zip"
    local SM_DIR="$(pwd)/QuantumROM/Mods/Apps/Samsung_SmartManagerCN_Android_${ANDROID_VERSION}"

    if [ ! -d "${EXTRACTED_FIRM_DIR}/system/system/priv-app/SmartManagerCN" ] && [ ! -f "$SM_ZIP" ]; then
        if curl -fsSL --connect-timeout 5 https://www.google.com >/dev/null; then
            echo "- Downloading SmartManagerCN for Android ${ANDROID_VERSION}..."
            wget --no-check-certificate \
                ...
                -O "$SM_ZIP" || rm -f "$SM_ZIP"
        else
            echo "- No internet. Skipping SmartManagerCN download."
        fi
    fi

    if [ ! -d "${EXTRACTED_FIRM_DIR}/system/system/priv-app/SmartManagerCN" ] && [ -f "$SM_ZIP" ]; then
        rm -rf "$SM_DIR"
        unzip -o "$SM_ZIP" -d "$SM_DIR" >/dev/null 2>&1
        ...
        cp -rfa "$SM_DIR/." "${EXTRACTED_FIRM_DIR}/"
    ...
    # Provides Magic Eraser / object / shadow / reflection remover models + PhotoEditor_AIFull.
    echo "- Adding Photo editor ai full (Magic Eraser / Photo Assist)."

    local PE_ZIP="$(pwd)/QuantumROM/Mods/Apps/Samsung_PhotoEditor_AIFull_Android_${ANDROID_VERSION}.zip"
    local PE_DIR="$(pwd)/QuantumROM/Mods/Apps/Samsung_PhotoEditor_AIFull_Android_${ANDROID_VERSION}"

    if [ ! -f "$PE_ZIP" ]; then
        if curl -fsSL --connect-timeout 5 https://www.google.com >/dev/null; then
            echo "- Downloading PhotoEditor AI Full for Android ${ANDROID_VERSION}..."
            wget --no-check-certificate \
                ...
                -O "$PE_ZIP" || rm -f "$PE_ZIP"
        else
            echo "- No internet. Place this file manually for Magic Eraser:"
            echo "  $PE_ZIP"
        fi
    fi

    # Always replace target PhotoEditor with the AI Full mod (A346E stock may ship a partial/broken build).
    if [ -f "$PE_ZIP" ]; then
        rm -rf "$PE_DIR"
        unzip -o "$PE_ZIP" -d "$PE_DIR" >/dev/null 2>&1
        ...
        cp -rfa "$PE_DIR/." "${EXTRACTED_FIRM_DIR}/"
        echo "- PhotoEditor AI Full installed."
    fi

    if [ ! -d "${EXTRACTED_FIRM_DIR}/system/system/priv-app/PhotoEditor_AIFull" ]; then
        echo "- WARNING: PhotoEditor_AIFull missing. Magic Eraser / Photo Assist will not work."
        echo "  Download and place:"
        echo "  https://github.com/SN-Abdullah-Al-Noman/Samsung_Special/releases/download/Android_${ANDROID_VERSION}/Samsung_PhotoEditor_AIFull_Android_${ANDROID_VERSION}.zip"
        echo "  into: QuantumROM/Mods/Apps/"
    fi

    # Fix PhotoEditor AI crash (missing model symlinks / GPU runtime on midrange).
    FIX_PHOTO_EDITOR_AI "$EXTRACTED_FIRM_DIR" || return 1

    # ================= OCR DATA PROVIDER =================
    echo "- Adding Samsung OCR Data Provider."

    local OCR_ZIP="$(pwd)/QuantumROM/Mods/Apps/Samsung_OCRDataProvider_Android_${ANDROID_VERSION}.zip"

    if [ ! -d "${EXTRACTED_FIRM_DIR}/system/system/app/OCRDataProvider" ] && [ ! -f "$OCR_ZIP" ]; then
        if curl -fsSL --connect-timeout 5 https://www.google.com >/dev/null; then
            echo "- Downloading OCRDataProvider for Android ${ANDROID_VERSION}..."
            wget --no-check-certificate \
                ...
                -O "$OCR_ZIP" || rm -f "$OCR_ZIP"
        else
            echo "- No internet. Skipping OCRDataProvider download."
        fi
    fi
...
APPLY_CUSTOM_FEATURES() {
	...
	# Software-side integrity spoof (Samsung Health / Knox "unauthorized changes").
	# Does not restore a hardware-tripped Knox warranty bit.
	BUILD_PROP "$EXTRACTED_FIRM_DIR" "system" "ro.boot.warranty_bit" "0"
	BUILD_PROP "$EXTRACTED_FIRM_DIR" "system" "ro.warranty_bit" "0"
	BUILD_PROP "$EXTRACTED_FIRM_DIR" "system" "ro.boot.verifiedbootstate" "green"
	BUILD_PROP "$EXTRACTED_FIRM_DIR" "system" "ro.boot.flash.locked" "1"
	BUILD_PROP "$EXTRACTED_FIRM_DIR" "system" "ro.boot.vbmeta.device_state" "locked"
	BUILD_PROP "$EXTRACTED_FIRM_DIR" "system" "ro.build.tags" "release-keys"
	...
	BUILD_PROP "$EXTRACTED_FIRM_DIR" "product" "ro.boot.warranty_bit" "0"
	BUILD_PROP "$EXTRACTED_FIRM_DIR" "product" "ro.warranty_bit" "0"
	BUILD_PROP "$EXTRACTED_FIRM_DIR" "product" "ro.boot.verifiedbootstate" "green"
	BUILD_PROP "$EXTRACTED_FIRM_DIR" "product" "ro.boot.flash.locked" "1"
	BUILD_PROP "$EXTRACTED_FIRM_DIR" "product" "ro.boot.vbmeta.device_state" "locked"
	BUILD_PROP "$EXTRACTED_FIRM_DIR" "product" "ro.build.tags" "release-keys"

	# Ensure stock in-display fingerprint type (A325F = optical_display_phone).
	# A165F source is capacitive_powerkey_phone — wrong for A325F UDFPS.
	if [ -n "$STOCK_ROM_FLOATING_FEATURE" ] && [ -f "$STOCK_ROM_FLOATING_FEATURE" ]; then
		local STOCK_FP_TYPE
		STOCK_FP_TYPE="$(GET_FF_VALUE "SEC_FLOATING_FEATURE_BIOAUTH_CONFIG_FINGERPRINT_FEATURES" "$STOCK_ROM_FLOATING_FEATURE")"
		if [ -n "$STOCK_FP_TYPE" ]; then
			echo "- Forcing fingerprint type from stock device: $STOCK_FP_TYPE"
			UPDATE_FLOATING_FEATURE "$FLOATING_FEATURE_FILE_DIRECTORY" \
				"SEC_FLOATING_FEATURE_BIOAUTH_CONFIG_FINGERPRINT_FEATURES" "$STOCK_FP_TYPE"
		fi
	fi
	...
	# Lock A325F panel limits (90Hz) and LowEnd launcher — overrides A346E 120Hz/HighEnd leftovers.
	APPLY_STOCK_DISPLAY_PERF_TUNING "$FLOATING_FEATURE_FILE_DIRECTORY"
